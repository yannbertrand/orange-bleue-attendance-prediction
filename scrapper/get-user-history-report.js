...
import { getUserVisits } from './models/get-user-visits.js';

export async function getUserHistoryReport() {
  const { authToken, cookie } = getOrangeBleueInfo();

  const response = await fetch(
    `https://monespace.lorangebleue.fr/nox/v1/studios/checkin/history/report`,
    {
      headers: {
  ...
  return getUserVisits(result);
}
console.log(await getUserHistoryReport());
